...
```{r profile_likelihood_m, fig.width= 6 , fig.height= 3}
...
```{r profile_likelihood_r, fig.width= 6, fig.height= 3}
...
Since our shape parameter is a function of our mean paramter, $\hat{r} = \dfrac{\hat{m}^2}{S^2 - \hat{m}}$ we can solve for our intervals by using the invariance property $(\theta_L,\theta_U)$ is a likelihood interval for $\theta$ such that $(g(\theta_L),g(\theta_U))$. This property does not hold for asymptotic intervals.

### Wald Interval  \label{WaldyBoi}
...
se_m <-  sqrt((-mle_negbinom$hessian)^(-1))

wald_m <- c(0,0)
wald_m[1] <- m_estimate-1.96*se_m
wald_m[2] <- m_estimate+1.96*se_m

#Assuming invariance property holds
...
data_frame('Parameter' = c("m","r"),'Lower bound'= c(wald_m[1],wald_r[1]), 'Upper bound' = c(wald_m[2],wald_r[2])) %>% knitr::kable(caption = "Wald intervals")
...
wilks_r <- c(0,0)
wilks_r[1] <- lower_bound_wilk_interval_r$root
wilks_r[2] <- upper_bound_wilk_interval_r$root

data_frame('Parameter' = c("m","r"),'Lower bound'= c(wilks_likelihood[1],wilks_r[1]), 'Upper bound' = c(wilks_likelihood[2],wilks_r[2])) %>% knitr::kable(caption = "Wilks Likelihood intervals")
...
Otherwise, for the case when the loglikelihood is not quadratic, the $\gamma$ value simply represents the lower bound for $\ell{(\theta_p)}$ values such that they are at least$\gamma$ times as likely as the MLE estimates in $\ell{(\hat{\theta_p})}$.
...
ggplot(df,aes(x = thetas,y = rthetas))+
  ...
  labs(x="m location parameter", y="Relative Likelihood")
...
As we can see the from the likelihood interval for $m$, all values within the interval [`r round(direct_interval_m[1],4)`,`r round(direct_interval_m[2],4)`] is at least 15% as likely as $\hat{m}$ = `r m_estimate`. Since this interval is not based on any underlying probability distribution of $\hat{m}$ thus it does not have any confidence level attached to it.

```{r,  fig.height =4,fig.width=5}
#likelihood ratio Graph for r

data_frame(x = r_range, val = r_profile_ratio) %>% 
    ggplot(aes(x=x,y=val))+
    geom_line()+
    geom_hline(yintercept = 0.15, col = "red", lty = "dashed")+
    theme(plot.title = element_text(hjust=0.5))+
    labs(title="Relative Likelihood for Negative Binomial") +
    labs(x="r shape parameter", y="Relative Likelihood")
...
#here is an example of how to do direct likelihood for r
...
data_frame('Lower bound'= direct_interval_r[1] , 'Upper bound' = direct_interval_r[2]) %>% knitr::kable(caption = " interval for r parameter")

```

The pure likelihood interval for $r$ requires no asymptotic assumptions or underlying probability distribution, it is entirely based on the data and thus will be our best estimate for the shape parameter. All values within the interval [`r round(direct_interval_r[1],4)`,`r round(direct_interval_r[2],4)`] is at least 15% as likely as $\hat{r}$ =`r r_estimate`.


#Results 

We now compare all the results attained from our intervals and compare them. The direct intervals have been displayed on a relatively likelihood plot of each parameter respectively. The other intervals shall be ommited from the visualisation due to high similarity and thus not easily visible.

```{r,fig.height =4,fig.width=5}
#relative likelihood with intervals for m

intervals_plot_m <- ggplot(df,aes(x = thetas,y = rthetas))+
  geom_line()+
  geom_vline(xintercept = direct_interval_m[1], col = "blue", lty = "dotted")+
  geom_vline(xintercept = direct_interval_m[2], col = "blue", lty = "dotted")+
  theme(plot.title = element_text(hjust=0.5))+
  labs(title="Direct Likelihood interval for m") +
  labs(x="m", y="Relative Likelihood")

intervals_plot_m
#gridExtra::grid.arrange(intervals_plot_m, intervals_plot_r,ncol=2)

```

```{r intervals}

intervals_m <- c("Wald Interval", "Wilks Likelihood","Direct Likelihood 15%")
lowerIntervals_m <- c(wald_m[1], wilks_likelihood[1], direct_interval_m[1])
upperIntervals_m <- c(wald_m[1], wilks_likelihood[2], direct_interval_m[2])

data_frame(Intervals = intervals_m, "Lower Bound" = lowerIntervals_m, "Upper Bound"= upperIntervals_m) %>% 
  knitr::kable(format = "latex", 
               caption = "Intervals for m parameter",
               booktabs = T)

# wald_m[1]-wald_m[2]
# wilks_likelihood[1]-wilks_likelihood[2]
# direct_interval_m[1]-direct_interval_m[2]
#we can see that wald is largest interval than wilks than direct

```

Our intervals for the $m$ parameter are very similar with the widest interval being the wald interval and narrowest being the pure likelihood interval. This means our asymptotic confidence interval for $m$ is [`r round(wald_m[1],4)`,`r round(wald_r[2],4)`] meaning that if more data was collected and each sample a confidence interval was collected, 95% of these intevals would contain our true parameter $m$. However, this does not give any cetainty on our particular interval actually containing the true parameter.

The narrowest interval, marginally, is our direct likelihood interval [`r round(direct_interval_m[1],4)`,`r round(direct_interval_r[2],4)`] which is not based on any underlying probability distribution but only that the interval for $m$ is 15% as likely as the MLE. The two intervals are in agreement.

```{r,fig.height =4,fig.width=5}
#relative likelihood with intervals for r

intervals_plot_r <- data_frame(x = r_range, val = r_profile_ratio) %>% 
    ggplot(aes(x=x,y=val))+
    geom_line()+
    geom_vline(xintercept = direct_interval_r[1], col = "darkgreen", lty = "dotted")+
    geom_vline(xintercept = direct_interval_r[2], col = "darkgreen", lty = "dotted")+
    theme(plot.title = element_text(hjust=0.5))+
    labs(title="Direct Likelihood interval for r") +
    labs(x="r shape parameter", y="Relative Likelihood")
intervals_plot_r
```
...
```{r}
intervals_r <- c("Wald Interval", "Wilks Likelihood","Direct Likelihood 15%")
lowerIntervals_r <- c(wald_r[1], wilks_r[1], direct_interval_r[1])
upperIntervals_r <- c(wald_r[2], wilks_r[2], direct_interval_r[2])

data_frame(Intervals = intervals_r, "Lower Bound" = lowerIntervals_r, "Upper Bound"= upperIntervals_r) %>% 
  knitr::kable(format = "latex", 
               caption = "Intervals for r parameter",
               booktabs = T)

# wald_r[1]-wald_r[2]
# wilks_r[1]-wilks_r[2]
# direct_interval_r[1]-direct_interval_r[2]
#we can see that wald is narrowest interval than wilks than direct
...
To further improve these models one could gather more data such that regression models could be used. This may yield more accurate results than simply trying to fit a distribution to the data. One could also explore different combinations of mixure models such as negative binomial, negative binomial or zero inflated negative binomial. These could potentially fit our data better, but The AIC, goodness of fit and asymptotic normality should be considered.
